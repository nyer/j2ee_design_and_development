...
<xsl:template match="item[@key='performance']">
	...
	<xsl:apply-templates select="format:dateTimeElement(when/time, $reqInfo/language, $reqInfo/country)"/>
	</b>
	<br/>
	<br/>
...
<xsl:template match="item[@key='reservation']">
	...
	<xsl:value-of select="format:currency(totalPrice, $reqInfo/language, $reqInfo/country)" />.
	This includes a booking fee of
	<xsl:value-of select="format:currency(quoteRequest/bookingFee, $reqInfo/language, $reqInfo/country)" />.
